...
bool CTerrain::init()
{
	...
	{
		//输出错误信息
		MessageBox(NULL, L"不支持多重纹理！", L"错误", MB_OK);
		exit(0);
		//setDetail(false);
	}
...
bool CTerrain::loadRawFile(const char* fileName)
{
	...
	{
		/** 输出错误信息，并返回false */
		MessageBox(NULL, L"打开高度图文件失败！",L"错误", MB_OK);
		exit(0);
	}
	...
	if (result)
	{
		MessageBox(NULL, L"无法获取高度数据！", L"错误", MB_OK);
		exit(0);
	}
...
bool CTerrain::loadTexture()
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		{
			MessageBox(NULL, L"装载位图文件失败！", L"错误", MB_OK);       /**< 如果载入失败则弹出对话框 */
			exit(0);
		}
